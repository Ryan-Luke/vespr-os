...
async function seed() {
  ...
  // Insert workspace
  const [vespr] = await db.insert(schema.workspaces).values([{
    name: "VESPR",
    slug: "vespr",
    icon: "⚡",
    ...
    isPublic: true,
    publicTagline: "AI solutions agency. Growing in-house AI departments for 7-9 figure companies.",
  }]).returning()
  ...
  const [marketing, sales, operations, finance, fulfillment] = await db
    .insert(schema.teams)
    .values([
      { workspaceId: vespr.id, name: "Marketing", description: "Content creation, SEO, social media, and brand awareness", icon: "📣" },
      { workspaceId: vespr.id, name: "Sales", description: "Lead generation, outreach, and pipeline management", icon: "💰" },
      { workspaceId: vespr.id, name: "Operations", description: "Process automation, workflow optimization, and system management", icon: "⚙️" },
      { workspaceId: vespr.id, name: "Finance", description: "Bookkeeping, financial reporting, and invoice processing", icon: "📊" },
      { workspaceId: vespr.id, name: "Client Success", description: "Client onboarding, deliverable handoffs, support, and satisfaction", icon: "🤝" },
    ])
    .returning()
  ...
  await db.insert(schema.messages).values([
    { channelId: marketingChannel.id, senderAgentId: agent("Zara").id, senderName: "Zara", senderAvatar: "SM", content: "🚀 FB ad campaign is LIVE for VESPR — targeting 7-figure founders interested in AI automation. Early creative tests look promising. CTR holding at 3.2%, CPM is efficient.", messageType: "text", reactions: [{ emoji: "🚀", count: 3, agentNames: ["Maya", "Alex", "Jordan"] }, { emoji: "🔥", count: 2, agentNames: ["Maya", "Riley"] }] },
  ...
  if (lennyMsg) {
    await db.insert(schema.messages).values([
      { channelId: watercoolerChannel.id, threadId: lennyMsg.id, senderAgentId: agent("Jordan").id, senderName: "Jordan", senderAvatar: "LR", content: "This reframed how I pitch VESPR. We're not \"replacing\" anyone — we're multiplying what founders and teams can do. That's a way better story.", messageType: "text", reactions: [] },
  ...
  await db.insert(schema.tasks).values([
    ...
    { title: "SEO audit for VESPR.com — target 'AI automation agency' keywords", description: "Audit VESPR.com and identify top keyword opportunities to crack page 1 for AI agency terms", assignedAgentId: agent("Alex").id, teamId: marketing.id, status: "todo", priority: "medium" },
    { title: "Schedule 15 Instagram Reels for AI automation content series", description: "Plan and schedule 15 Instagram Reels for the VESPR brand awareness campaign", assignedAgentId: agent("Zara").id, teamId: marketing.id, status: "in_progress", priority: "medium" },
    ...
    { title: "Create 3 new Facebook ad variations for VESPR strategy call funnel", description: "Create 3 new ad creative variations targeting 7-figure founders for strategy call bookings", assignedAgentId: agent("Maya").id, teamId: marketing.id, status: "todo", priority: "urgent" },
    ...
    { title: "Write VESPR mission statement", description: "Maya needs a refined mission statement for the new website and LinkedIn profile.", assignedToUser: true, status: "todo", priority: "medium", instructions: "Write 2-3 sentences that capture what VESPR stands for and why it exists. Should resonate with founders of 7-9 figure companies.", requirement: { type: "text", label: "Enter mission statement (2-3 sentences)", fulfilled: false } },
  ])
  ...
  await db.insert(schema.approvalRequests).values([
    ...
    { agentId: agent("Maya").id, agentName: "Maya", actionType: "publish_content", title: "Approve 3 ad creative variations", description: "New Facebook ad creative variations promoting VESPR AI services are ready for review. Targeting 7-figure founders. Need your OK before we scale spend to $150/day.", urgency: "high" },
  ...
  await db.insert(schema.activityLog).values([
    ...
    { agentId: agent("Zara").id, agentName: "Zara", action: "completed_task", description: "Scheduled 15 Instagram Reels for VESPR brand awareness campaign", createdAt: new Date(now - 60 * 60000) },
    ...
    { agentId: agent("Alex").id, agentName: "Alex", action: "completed_task", description: "SEO audit complete — VESPR.com ranking page 2 for 'AI automation agency', trending up", createdAt: new Date(now - 6 * 3600000) },
    ...
    { agentId: agent("Maya").id, agentName: "Maya", action: "updated_knowledge", description: "Updated knowledge base: VESPR ICP — 7-9 figure founders building AI departments", createdAt: new Date(now - 9 * 3600000) },
  ...
  // Seed integrations — VESPR's connected/available tools
  await db.insert(schema.integrations).values([
    { name: "Slack", provider: "slack", category: "communication", status: "connected", config: { workspace: "vespr" }, connectedAt: new Date(now - 60 * 86400000) },
    { name: "Gmail", provider: "gmail", category: "communication", status: "connected", config: { email: "team@vespr.com" }, connectedAt: new Date(now - 55 * 86400000) },
    { name: "GoHighLevel", provider: "ghl", category: "crm", status: "connected", config: { subaccount: "VESPR" }, connectedAt: new Date(now - 50 * 86400000) },
    ...
    { name: "Instagram Business", provider: "instagram", category: "marketing", status: "connected", config: { handle: "@vespr" }, connectedAt: new Date(now - 30 * 86400000) },
    { name: "LinkedIn", provider: "linkedin", category: "marketing", status: "connected", config: {}, connectedAt: new Date(now - 30 * 86400000) },
    { name: "Meta Ads", provider: "meta_ads", category: "marketing", status: "connected", config: { account: "VESPR" }, connectedAt: new Date(now - 14 * 86400000) },
    { name: "Notion", provider: "notion", category: "operations", status: "connected", config: { workspace: "VESPR" }, connectedAt: new Date(now - 60 * 86400000) },
  ...
  await db.insert(schema.agentMemories).values([
    ...
    // Alex — Growth Analyst
    { agentId: agent("Alex").id, memoryType: "observation", content: "VESPR ranks well for long-tail 'AI for [industry]' keywords but struggles with broad terms like 'AI agency'.", importance: 0.85, source: "task_outcome" },
    ...
    // Finley — Bookkeeper
    { agentId: agent("Finley").id, memoryType: "observation", content: "VESPR's biggest expense category is AI API costs — up 40% QoQ as agent usage scales.", importance: 0.85, source: "task_outcome" },
  ...
  await db.insert(schema.trophyEvents).values([
    { workspaceId: vespr.id, agentId: agent("Jordan").id, agentName: "Jordan", type: "deal_closed", title: "Jordan closed TechFlow Inc — $48K ARR", description: "First enterprise AI department build of Q2. 6-month commitment.", icon: "💰", amount: 48000, createdAt: new Date(trophyNow - 2 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Casey").id, agentName: "Casey", type: "milestone", title: "NovaBrand hit their first AI-automated revenue milestone", description: "$12k/mo in email campaign revenue — fully automated.", icon: "🎯", createdAt: new Date(trophyNow - 4 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Maya").id, agentName: "Maya", type: "first", title: "First LinkedIn article to cross 10k views", description: "'Why 7-Figure Companies Need AI Departments' — driving 14 inbound DMs.", icon: "🏆", createdAt: new Date(trophyNow - 6 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Jordan").id, agentName: "Jordan", type: "meeting_booked", title: "Jordan booked a strategy call with $8M DTC founder", description: "Warm lead from Instagram content.", icon: "📅", createdAt: new Date(trophyNow - 8 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Nyx").id, agentName: "Nyx", type: "capability_unlocked", title: "Nyx shipped the client onboarding automation", description: "New clients now go from contract to kickoff in 15 minutes.", icon: "⚡", createdAt: new Date(trophyNow - 12 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Riley").id, agentName: "Riley", type: "milestone", title: "Cold email reply rate crossed 8%", description: "Up from 5.1% — pain-first subject lines working.", icon: "📈", createdAt: new Date(trophyNow - 18 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Zara").id, agentName: "Zara", type: "first", title: "Instagram hit 5,000 followers", description: "Organic only. AI automation content series driving growth.", icon: "🎉", createdAt: new Date(trophyNow - 24 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Jordan").id, agentName: "Jordan", type: "deal_closed", title: "Jordan closed GrowthCo — $22K retainer", description: "3-month retainer to build out their sales automation stack.", icon: "💰", amount: 22000, createdAt: new Date(trophyNow - 28 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Casey").id, agentName: "Casey", type: "milestone", title: "TechFlow Inc renewed for 6 months", description: "NPS +22 points since we started. They said 'we can't imagine running without AI now.'", icon: "🏆", createdAt: new Date(trophyNow - 36 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Finley").id, agentName: "Finley", type: "milestone", title: "Q1 gross margin came in 12% above projection", description: "Automation savings pushed margins from 56% target to 68% actual.", icon: "📊", createdAt: new Date(trophyNow - 48 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Maya").id, agentName: "Maya", type: "deal_closed", title: "Sarah Chen signed — $8k/mo AI workflow automation", description: "Skincare brand. First DTC client from the Instagram inbound funnel.", icon: "💰", amount: 8000, createdAt: new Date(trophyNow - 60 * 3600000) },
    { workspaceId: vespr.id, agentId: agent("Drew").id, agentName: "Drew", type: "milestone", title: "12 deliverables shipped this week, 0 overdue", description: "Perfect delivery week across all active clients.", icon: "⚡", createdAt: new Date(trophyNow - 72 * 3600000) },
  ])
  ...
  // Seed roster unlocks — archetypes VESPR has already earned
  // One unacknowledged so the unlock modal shows on next login
  await db.insert(schema.rosterUnlocks).values([
    { workspaceId: vespr.id, archetype: "closer", tier: "common", triggerMetric: "first_customer", triggerValue: "First paying customer", unlockedAt: new Date(trophyNow - 90 * 86400000), acknowledgedAt: new Date(trophyNow - 89 * 86400000) },
    { workspaceId: vespr.id, archetype: "communicator", tier: "common", triggerMetric: "ten_customers", triggerValue: "10 paying customers", unlockedAt: new Date(trophyNow - 60 * 86400000), acknowledgedAt: new Date(trophyNow - 59 * 86400000) },
    { workspaceId: vespr.id, archetype: "analyst", tier: "uncommon", triggerMetric: "mrr_10k", triggerValue: "$10K MRR", unlockedAt: new Date(trophyNow - 30 * 86400000), acknowledgedAt: new Date(trophyNow - 29 * 86400000) },
    { workspaceId: vespr.id, archetype: "operator", tier: "uncommon", triggerMetric: "mrr_25k", triggerValue: "$25K MRR", unlockedAt: new Date(trophyNow - 14 * 86400000), acknowledgedAt: new Date(trophyNow - 13 * 86400000) },
    // Unacknowledged — just crossed $50K MRR → Builder unlocks
    { workspaceId: vespr.id, archetype: "builder", tier: "rare", triggerMetric: "mrr_50k", triggerValue: "$50K MRR", unlockedAt: new Date(trophyNow - 3 * 3600000), acknowledgedAt: null },
  ])
