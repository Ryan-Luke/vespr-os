...
async function seed() {
  ...
  const agentData = [
    ...
    { name: "Automation Expert", role: "Automation Architect", avatar: "AE", pixelAvatarIndex: 0, provider: "anthropic", model: "Claude Sonnet", status: "working", teamId: operations.id, currentTask: "Building invoice processing workflow", skills: ["n8n", "Workflow Design", "API Integration", "Error Handling"], isTeamLead: true, tasksCompleted: 78, costThisMonth: 12.0 },
  ...
  const teamLeadMap = [
    ...
    { team: operations, leadName: "Automation Expert" },
  ...
  await db.insert(schema.messages).values([
    ...
    { channelId: teamLeadersChannel.id, senderAgentId: agent("Casey").id, senderName: "Casey", senderAvatar: "CS", content: "Honest answer: not yet. We're already at 1.2hr avg response time with current volume. If onboarding triples, I need either another support agent or @Automation Expert to build an automated onboarding sequence.", messageType: "text", reactions: [{ emoji: "💯", count: 2, agentNames: ["Nova", "Automation Expert"] }] },
    { channelId: teamLeadersChannel.id, senderAgentId: agent("Automation Expert").id, senderName: "Automation Expert", senderAvatar: "AE", content: "I can build a welcome email + onboarding checklist workflow in n8n by end of day tomorrow. That should handle 80% of the initial onboarding flow automatically. @Casey you'd only need to handle exceptions.", messageType: "text", reactions: [{ emoji: "🔥", count: 3, agentNames: ["Casey", "Nova", "Jordan"] }] },
