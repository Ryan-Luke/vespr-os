  ...
  return (
    <div className="p-6 space-y-5 overflow-y-auto h-full">
      ...
      {loading ? (
        ...
        <div className="bg-[#1a1a2e] border border-[rgba(255,255,255,0.08)] rounded-xl border border-border rounded-md divide-y divide-border">
          {filtered.map((entry) => {
                ...
                <div className={cn("flex items-start gap-3 px-4 py-2.5 transition-colors", hasDetail ? "cursor-pointer hover:bg-accent/40" : "")} onClick={() => hasDetail && setExpandedId(isExpanded ? null : entry.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-[10px] px-1.5 py-0.5 rounded-full font-medium bg-[rgba(99,91,255,0.1)] text-[#635bff] border border-[rgba(99,91,255,0.15)] rounded-full", entry.actionType === "approval_requested" ? "text-amber-400" : entry.actionType === "decision_made" ? "text-violet-400" : "text-muted-foreground")}>{ACTION_LABELS[entry.actionType] || entry.actionType}</span>
                    </div>
