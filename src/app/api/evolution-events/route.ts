...
export async function GET(req: Request) {
  ...
  const rows = onlyUnacknowledged
    ? await db.select().from(evolutionEvents).where(isNull(evolutionEvents.acknowledgedAt)).orderBy(desc(evolutionEvents.occurredAt))
    : await db.select().from(evolutionEvents).orderBy(desc(evolutionEvents.occurredAt))
...
export async function PATCH(req: Request) {
  ...
  await db.update(evolutionEvents).set({ acknowledgedAt: new Date() }).where(eq(evolutionEvents.id, id))
